...
function2 <- function(data, group){
  data %>% 
    ...
    rename("Variable" = "Get(Group)") %>%
    knitr::kable(format = "markdown")
}

function2(coffee_ratings, "country_of_origin")
```
...
coffee_ratings_clean <- coffee_ratings %>%
  mutate(across(everything(), 
                str_replace_na, 
                "Missing"))
coffee_ratings_clean$altitude_mean_meters[coffee_ratings_clean$altitude_mean_meters > 8848] <- "Missing"
cut2()
...
coffee_ratings_clean %>%
  map_dfr(extract, c("species", "county_of_origin", "binned_mean_altitude"))
```
...
scatterplot <- function(var_1, var_2, color){
  ggplot(coffee_ratings, aes(x = get(var_1),
                             y = get(var_2),
                             col = color)) +
...
scatterplot("aroma", "moisture", "color")
```